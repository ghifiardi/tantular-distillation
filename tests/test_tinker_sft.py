...
def test_the_edit_contract_scorer_keeps_its_scratch_file_out_of_the_repo():
    """Running the suite must not leave artifacts in the working tree.

    The scorer used to write data/gates/fce/_cases.json — a fixed path inside
    the repository, so the tests dirtied the tree and two concurrent scorer
    runs shared one cases file.
    """
    import inspect
    import score_faithful_edit

    source = inspect.getsource(score_faithful_edit.run_contract_checker)
    # Ignore comments: the rationale above the fix names the old path.
    code = "\n".join(line for line in source.splitlines()
                     if not line.lstrip().startswith("#"))
    assert "tempfile.TemporaryDirectory" in code
    assert "_cases.json" not in code
    assert '"gates"' not in code
